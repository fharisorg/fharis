...
function fifu_connect_social(socialMedia, enabled) {
    const popupName = `fifu-social-${socialMedia}`;
    const popupFeatures = 'width=720,height=760,menubar=no,toolbar=no,location=no,status=no,scrollbars=yes,resizable=yes';
    let popup = null;

    try {
        popup = window.open('', popupName, popupFeatures);
    } catch (err) {
        console.error('Popup blocked, falling back to new tab', err);
    }
    ...
    if (popup) {
        const injectPlaceholder = () => {
            try {
                ...
                doc.body.innerHTML = '<p style="font-family:Arial, sans-serif;font-size:14px;padding:16px;margin:0;">Loading…</p>';
            } catch (placeholderErr) {
                console.debug('Unable to render popup placeholder', placeholderErr);
            }
        };
        ...
        try {
            if (popup.document && popup.document.readyState === 'complete') {
                ...
                popup.addEventListener('DOMContentLoaded', injectPlaceholder, {once: true});
            }
        } catch (err) {
            console.debug('Unable to access popup document before navigation', err);
        }
    } else {
    ...
    jQuery.ajax({
        method: "POST",
        url: restUrl + 'fifu-premium/v2/connect-social-api/',
        dataType: 'json',
        beforeSend: xhr => xhr.setRequestHeader('X-WP-Nonce', fifuScriptVars.nonce),
        data: {socialMedia, enabled},
        success: data => {
            const target = (data && (data.launch_url || data.dialog_url)) ? (data.launch_url || data.dialog_url) : null;
            if (!target) {
        ...
        error: (jqXHR, textStatus, errorThrown) => {
            if (popup && !popup.closed)
                popup.close();
            console.error('OAuth init failed', {jqXHR, textStatus, errorThrown});
        },
        complete: () => jQuery('#tabs-top').unblock(),
        timeout: 30000
    });
}
